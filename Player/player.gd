...
@onready var javelin_base = get_node("Attack/JavelinBase")
...
func spawn_javelin() -> void:
	var get_javelin_total = javelin_base.get_child_count() # get total javelins
	var calc_spawns = javelin_ammo - get_javelin_total # eg. 3 - 0 = 3
	while calc_spawns > 0:
